import { entities, href, segments, translators, translatorHref } from '../lib/data';
export function GET() {
 const paths=['/','/map/','/search/','/resources/','/about/','/translators/',...translators.map(t=>translatorHref(t.id)),...new Set(entities.map(e=>`/${segments[e.type]}/`)),...entities.map(href)];
